...
#'
#' The \code{blocked weighted bootstrap (BWB)} is an estimation technique for use with
#' data from two-stage cluster sampled surveys in which either prior weighting
#' (e.g. \code{population proportional sampling} or \code{PPS} as used in
#' \code{SMART} surveys) or posterior weighting (e.g. as used in
...
#' percentile bootstrap to include \code{blocking} and \code{weighing} to
...
#'
#' A total of \em{m} \code{PSUs} are sampled with replacement for each
#' bootstrap replicate (where \em{m} is the number of \code{PSUs} in the survey
#' sample).
#'
